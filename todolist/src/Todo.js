...
const Todo = () => {
  ...
  const handleaddtitle = (event) => 
  {
    ...
    axios.post("http://localhost:3001/posts", { title:list,isPublished:false })
      .then((response) => {
  ...
  const handleSaveEdit = (id) => {
    ...
    axios
      .put(`http://localhost:3001/posts/${id}`, { title: editedTitle })
      .then((response) => {
        if (response.status === 200) {
          fetchtitles();
          setEditableId(null);
...
//navigate to anotherpage for edit
// import React, { useEffect, useState } from 'react';
// import {Table,Button} from 'react-bootstrap';
// import { FaUserEdit} from "react-icons/fa";
// import { MdDelete } from "react-icons/md";
...
//   //   axios.put(`http://localhost:3001/posts/${id}`,formdata)
//   //   .then((response)=>{
//   //     if(response.status===200)
//   //     {
//   //      fetchtitles();
